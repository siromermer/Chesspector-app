...
class _CornerDetectionPageState extends State<CornerDetectionPage> {
  ...
  String? _generateFenFromDetection() {
    ...
    for (final piece in _detectedPieces!) {
      ...
      if (fenChar != null) {
        ...
        if (board[row][col] == null || conf > boardConf[row][col]) {
          debugPrint('Placing $fenChar at ($row,$col) with conf=$conf ${board[row][col] != null ? "(replacing ${board[row][col]})" : ""}');
          board[row][col] = fenChar;
          boardConf[row][col] = conf;
          
          // Only track this piece for post-processing if it made it onto the board
          ...
          debugPrint('Skipping $fenChar at ($row,$col) conf=$conf (keeping ${board[row][col]} with conf=${boardConf[row][col]})');
        }
      }
    }

    // Rule 1: Each side MUST have exactly one king.
    debugPrint('Before king correction: white K=${whiteKings.length} Q=${whiteQueens.length} B=${whiteBishops.length}');
    debugPrint('Before king correction: black k=${blackKings.length} q=${blackQueens.length} b=${blackBishops.length}');
    ...
    debugPrint('After king correction: white has ${board.expand((r) => r).where((p) => p == 'K').length} king(s)');
    debugPrint('After king correction: black has ${board.expand((r) => r).where((p) => p == 'k').length} king(s)');
  ...
  void _ensureOneKing(
    ...
    if (kings.isEmpty) {
      // No king detected — promote the lowest-conf queen, else lowest-conf bishop
      debugPrint('${isWhite ? "White" : "Black"} has no king. Queens: ${queens.length}, Bishops: ${bishops.length}');
      if (queens.isNotEmpty) {
        queens.sort((a, c) => a.conf.compareTo(c.conf));
        final p = queens.removeAt(0);
        debugPrint('  → Promoting queen at (${p.row},${p.col}) conf=${p.conf} to king');
        board[p.row][p.col] = k;
      } else if (bishops.isNotEmpty) {
        bishops.sort((a, c) => a.conf.compareTo(c.conf));
        final p = bishops.removeAt(0);
        debugPrint('  → Promoting bishop at (${p.row},${p.col}) conf=${p.conf} to king');
        board[p.row][p.col] = k;
      } else {
        debugPrint('  → ERROR: No queens or bishops to promote!');
      }
    } else if (kings.length >= 2) {
      // Too many kings — keep highest conf, demote the rest
      debugPrint('${isWhite ? "White" : "Black"} has ${kings.length} kings. Keeping highest conf, demoting others.');
      kings.sort((a, c) => c.conf.compareTo(a.conf));
      debugPrint('  → Keeping king at (${kings[0].row},${kings[0].col}) conf=${kings[0].conf}');
      final bool hasQueen = queens.isNotEmpty;
      for (int i = 1; i < kings.length; i++) {
        final extra = kings[i];
        if (!hasQueen && i == 1) {
          debugPrint('  → Demoting extra king at (${extra.row},${extra.col}) to queen');
          board[extra.row][extra.col] = q;
        } else {
          debugPrint('  → Demoting extra king at (${extra.row},${extra.col}) to bishop');
